...
|---|---|---|---|---|
| P1 Sukurti marsruta | Open route creation, enter start/end cities | `frontend/src/Views/Travel/RoutesPage.tsx` | `POST /api/Route/openRouteCreation`, `POST /api/Route` -> `RouteController.saveRoute` | `checkRoute`, `sendCities`, `checkCities`, `saveRouteData` |
...
| P1 Sukurti marsruta | Calculate route length matrix and order | `RoutesPage.saveRoute` | `RouteController.saveRoute` | `createLengthMatrix`, `createGraph`, `calculateHeuristic`, `createInitialRoute`, `shuffleObjectOrder`, `selectRoad` |
| P1 Sukurti marsruta | Create route geometry and save result | `RouteMap` polyline display | `RouteController.saveRoute` | `createPolyLine`, `checkPolyLine`, `createFallbackPolyLine`, `saveRouteTime`, `saveRouteData` |
